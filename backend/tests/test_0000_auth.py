...
def test_create_same_user(test_app):
    user_data = ADMIN_USER_DATA
    response = test_app.post("/register", json=user_data)
    assert response.status_code == 400
    assert response.json()['detail'] == "Este correo ya está registrado."

def test_correct_login_user(test_app):
    user_data = {
...
def test_login_user_wrong_password(test_app):
    user_data = {
        "email": ADMIN_USER_DATA["email"],
        "password": "wrong"
    }
...
def test_read_user_no_token(test_app):
    response = test_app.get("/users/me")
    assert response.status_code == 403
    assert response.json()['detail'] == "Not authenticated"

def test_read_user_invalid_token(test_app):
    headers = {
